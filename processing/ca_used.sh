...
echo "$certs" | jq -c '[.[] | { "domain" : .domain, "chain" : .tls_connection_info.cert_chain.chain[-1].issuer_name }] | unique | .[] | .chain' | sort -n | uniq -c | sort -rn | head -n 10 | sed -E 's/[ \t]*([0-9]+)[ ]/\1=/' | awk -v tot=$total 'BEGIN {FS="="; OFS="\t"; printf("%100s | %5s | %s\n", "CA", "Count", "%X")} {printf("%100s | %5s | %.3f\n", $2, $1, ($1/tot))}'
